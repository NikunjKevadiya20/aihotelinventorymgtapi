    ...
    public class BookingLookupRepository : IBookingLookupRepositoryInterface
    {
            ...
            string storedProcedure)
        {
            ...
            using SqlConnection sqlConnection =
                new SqlConnection(CommonRepositoryConstants.DefaultConnectionString);
            ...
            try
            {
                ...
                dynamicParameters.Add("@CompanyAddress", entity.CompanyAddress);

                dynamicParameters.Add("@Price", entity.Price);
                dynamicParameters.Add("@DiscountValue", entity.DiscountValue);
                dynamicParameters.Add("@ServiceCharge", entity.ServiceCharge);
                dynamicParameters.Add("@FinalPrice", entity.FinalPrice);
                dynamicParameters.Add("@TotalRooms", entity.TotalRooms);
                dynamicParameters.Add("@TotalTickets", entity.TotalTickets);
                dynamicParameters.Add("@StateID", entity.StateID);
                dynamicParameters.Add("@DistrictID", entity.DistrictID);
                dynamicParameters.Add("@WhatsAppMobileNo", entity.WhatsAppMobileNo);
                dynamicParameters.Add("@RoundOff", entity.RoundOff);
                ...
                dynamicParameters.Add("@TicketDate", entity.TicketDate);
                dynamicParameters.Add("@CreatedBy", entity.CreatedBy);

                // ==========================================
                // TEMP TICKET LIST
                // ==========================================
                List<TempTicketType> tempTicketTypes =
                    new List<TempTicketType>();

                // ==========================================
                // TEMP TICKET PAX LIST
                // ==========================================
                List<TempTicketPaxType> tempTicketPaxTypes =
                    new List<TempTicketPaxType>();
                ...
                if (entity.TicketCart != null)
                {
                    foreach (var item in entity.TicketCart)
                    {
                        tempTicketTypes.Add(new TempTicketType
                        {
                            SrNo = srNo,
                            TicketID = item.TicketID,
                            TimeSlotID = item.TimeSlotID,
                            ProjectionMappingID = item.ProjectionMappingID,
                            Price = item.Price,
                            ServiceCharge = item.ServiceCharge,
                            TotalPrice = item.TotalPrice,
                            NoOfTicket = item.NoOfTicket,
                            TotalChildSevenYr = item.TotalChildSevenYr,
                            TotalChildThreeYr = item.TotalChildThreeYr,
                            TotalAdults = item.TotalAdults,
                            SlotIDs = item.SlotIDs
                        });

                        // Pax details
                        if (item.TicketPaxDetails != null)
                        {
                            foreach (var pax in item.TicketPaxDetails)
                            {
                                tempTicketPaxTypes.Add(
                                    new TempTicketPaxType
                                    {
                                        TicketSrNo = srNo,
                                        Name = pax.Name,
                                        Age = pax.Age,
                ...
                var data = await sqlConnection.QueryFirstOrDefaultAsync<
                    TempBookingIDViewEntity>(
                    storedProcedure,
                    dynamicParameters,
        ...
        public async Task<BookingViewInsertEntity> InsertBooking(BookingRequestEntity entity, string storedProcedure)
        {
            ...
            try
            {
                ...
                {
                    ticket.PaxList = PaxList.Where(p => p.BookingTicketID == ticket.ID).ToList();
                }


                foreach (var bookingTicket in TicketView)
                {
                    var slotIds = (bookingTicket.SlotIDs ?? "")
                        .Split(',', StringSplitOptions.RemoveEmptyEntries)
                        .Select(x => Convert.ToInt32(x.Trim()))
                        .ToList();

                    var attractions = AttractionTicket
                        .Where(a => a.TicketID == bookingTicket.TicketID)
                        .ToList();
                    ...
                    foreach (var attraction in attractions)
                    {
                        attraction.AttractionTicketChild = AttractionTicketChild
                            .Where(c =>
                                c.TicketID == bookingTicket.TicketID &&
                                c.AttractionID == attraction.ID &&
                                slotIds.Contains(c.ID ?? 0))
                            .ToList();
                    }

                    bookingTicket.AttractionTicket = attractions;
                }
                ...
                if (booking.Message?.Trim().ToLower() == "success" && !string.IsNullOrWhiteSpace(booking.EmailID))
                {
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            bool isPackageBooking = booking.HotelBookingPackageID.HasValue && booking.HotelBookingPackageID > 0;

                            if (isPackageBooking)
                            {
                                // Package booking - use HotelBookingPackageName
                                string packageName = booking.HotelBookingPackageName ?? "Package";
                                customerSubject = $"{pnr} || CONFIRM BOOKING || {packageName} || {fullName}";
                            }
                            else if (hasTickets && !hasHotel)
                            {
                                // Only tickets (no hotel, no package)
                                customerSubject = $"{pnr} || CONFIRM BOOKING || {ticketName} || {fullName}";
                            }
                            else if (hasHotel && !hasTickets)
                            {
                                // Only hotel (no tickets, no package)
                                customerSubject = $"{pnr} || CONFIRM BOOKING || {hotelName} || {fullName}";
                            }
                            else // Both tickets and hotel (non-package)
                            {
...
<body>
    ...
    <td class='value' style='color:#2563eb; font-weight:700;'>&#8377; {finalPrice:N0}</td>
                            ...
                            // ==================== TICKET SECTION (Multiple Tickets / Package) ====================
                            if (hasTickets)
                            {
                                customerHtml += @"
            <!-- Package / Ticket Details -->";
                                ...
                                foreach (var ticket in TicketView)
                                {
                                    bool isPackage = booking.HotelBookingPackageID.HasValue && booking.HotelBookingPackageID > 0;

                                    string displayName = isPackage
                                        ? (booking.HotelBookingPackageName ?? ticket.HotelBookingTicketName ?? "Package")
                                        : (ticket.HotelBookingTicketName ?? "Activity");

                                    string sectionTitle = isPackage
                                        ? "Package Details"
                                        : "Ticket Details";

                                    customerHtml += $@"
            <div class='section-title'>{sectionTitle} : {displayName}</div>
            <table style='margin-bottom: 20px;'>
                <tr>
                    <td class='label'>{(isPackage ? "Package Name" : "Ticket Name")}</td>
                    <td><strong>{displayName}</strong></td>
                </tr>";

                                    // Dynamic Attraction Details
                                    if (ticket.AttractionTicket?.Any() == true)
                                    {
                                        foreach (var attraction in ticket.AttractionTicket)
                                        {
                                            if (attraction.AttractionTicketChild?.Any() == true)
                                            {
                                                string slotText = string.Join(", ",
                                                    attraction.AttractionTicketChild
                                                ...
                                                if (!string.IsNullOrWhiteSpace(slotText))
                                                {
                                                    customerHtml += $@"
                                                    <tr>
                                                        <td class='label'>{attraction.Title}</td>
                                                        <td>{slotText}</td>
                                                    </tr>";
                                                }
                                            }
                ...
                <tr><td class='label'>No. of Tickets</td><td>{ticket.NoOfTicket ?? 1}</td></tr>";
                                    ...
                                    // Total Price - Show ONLY for Normal Tickets (Hide for Packages)
                                    if (!isPackage && ticket.TotalPrice.HasValue && ticket.TotalPrice > 0)
                                    {
                                        customerHtml += $@"
                                <tr><td class='label'>Total Price</td><td>&#8377; {ticket.TotalPrice:N0}</td></tr>";
                                    }
                                    else if (!isPackage && booking.FinalPrice.HasValue && booking.FinalPrice.Value > 0)
                                    {
                                        // Fallback for normal tickets
                                        customerHtml += $@"
                                <tr><td class='label'>Total Price</td><td>&#8377; {booking.FinalPrice:N0}</td></tr>";
                                    }
                    ...
                    <table>
                        ...
                        <tr><td class='label'>Check-In</td><td>{HotelDetails.FirstOrDefault()?.CheckIn}</td></tr>
                        <tr><td class='label'>Check-Out</td><td>{HotelDetails.FirstOrDefault()?.CheckOut} </td></tr>
                        ...
                        <tr><td class='label'>Extra Adult</td><td>{hotel.ExtraAdult}</td></tr>";
                                ...
                                // Show Total Amount ONLY if it is NOT a Package
                                if (!isPackage && hotel.TotalAmount != null)
                                {
                                    customerHtml += $@"
                <tr><td class='label'>Total Amount</td><td>&#8377; {(hotel.TotalAmount ?? 0):N0}</td></tr>";
                                }
    ...
    <div class='footer-bar'>
        ...
        <div class='footer-bar-right'>
            ...
            <div class='footer-help'>Need Help? +91 9898 034 096</div>
                        ...
                        {
                            // Log if you have logger in scope
                        }
                    });
                }
            ...
            {
                logger.LogError(sqlEx, sqlEx.Message);
        ...
        public async Task<List<BookingListEntity>> FindAllBooking(BookingSearchEntity entity, string storedProcedure)
        {
            try
            {
                ...
                dynamicParameters.Add("@PNR", entity.PNR);
        ...
        public async Task<BookingViewEntity> FindByIDBooking(BookingEntity entity, string storedProcedure)
        {
            BookingViewEntity result = new BookingViewEntity();
            ...
            try
            {
                ...
                var order = await data.ReadFirstOrDefaultAsync<BookingViewEntity>();

                if (order != null)
                {
                    // 2. TICKETS
                    var tickets = (await data.ReadAsync<BookingTicketViewEntity>()).ToList();

                    // 3. PAX
                    var pax = (await data.ReadAsync<BookingTicketPaxViewEntity>()).ToList();

                    // 4. HOTEL
                    var hotels = (await data.ReadAsync<BookingHotelDetailsViewEntity>()).ToList();
                    var AttractionTicket = (await data.ReadAsync<AttractionTicket>()).ToList();
                    var AttractionTicketChild = (await data.ReadAsync<AttractionTicketChild>()).ToList();
