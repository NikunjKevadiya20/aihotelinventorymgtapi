    ...
    public class BookingLookupRepository : IBookingLookupRepositoryInterface
    {
        ...
        public async Task<TempBookingIDViewEntity> InsertTempBooking(TempBookingEntity entity, string storedProcedure)
        {
            ...
            try
            {
                ...
                dynamicParameters.Add("@someoneElseFirstName", entity.someoneElseFirstName);          
                dynamicParameters.Add("@someoneElseLastName", entity.someoneElseLastName);          
                ...
                if (booking != null)
                {
                    ...
                    if (booking.Message?.Trim().ToLower() == "success" && !string.IsNullOrWhiteSpace(booking.EmailID))
                    {
                        _ = Task.Run(async () =>
                        {
                            try
                            {
                                string companyName = booking.CompanyInfo?.PropertyName;
                                ...
                                // ==================== Logo from CompanyProfile.Image ====================
                                string imageUrl = booking.CompanyInfo?.Image;

                                // For EMAIL - use direct URL (Gmail friendly)
                                string emailLogoHtml = $"<span style='color:white; font-size:22px; font-weight:bold;'>{companyName}</span>";
        ...
        <img src='{imageUrl}' 
             ...
             style='max-height:52px; max-width:180px; height:auto; display:block; margin:0 auto;' />";
                                }

                                // For PDF - use base64 (Puppeteer friendly)
                                string pdfLogoHtml = emailLogoHtml; // fallback
                                ...
                                if (!string.IsNullOrWhiteSpace(imageUrl))
                                {
                                    try
                                    {
                                        ...
                                        string mime = GetMimeType(imageUrl);

                                        pdfLogoHtml = $@"
            <img src='data:{mime};base64,{base64}' 
                 alt='{companyName} Logo' 
                 style='max-height:48px; max-width:180px; height:auto; display:block;' />";
                                    }
                                    catch
                                    {
                                        // keep fallback
...
<tr>
    <td style='background:#0f172a; padding:22px 28px;'>
        <table width='100%' cellpadding='0' cellspacing='0' border='0'>
            <tr>
                <!-- Logo (Left) -->
                <td width='50%' align='left' valign='middle' style='padding:0;'>
                    {emailLogoHtml}
                </td>

                <!-- Title (Right) -->
                <td width='50%' align='right' valign='middle' style='padding:0; color:#ffffff;'>
                    <div style='font-size:20px; font-weight:700; line-height:1.3; color:#ffffff; letter-spacing:0.5px;'>
                        BOOKING CONFIRMED
                    </div>
                    <div style='margin-top:6px; font-size:13px; color:#ffffff; opacity:0.9;'>
...
<table class='header-table' cellpadding='0' cellspacing='0' border='0' width='100%'>
    <tr>
        <!-- Logo -->
        <td class='header-left' width='50%' style='padding:22px 28px; vertical-align:middle;'>
            {pdfLogoHtml}
        </td>
    ...
    <table width='100%' cellpadding='0' cellspacing='0' border='0' style='background:#f3f4f6; padding:30px 0;'>
        ...
        public async Task<ResultModel> InsertPaymentData(PaymentDataEntity entity, string storedProcedure)
        {
            ...
            try
            {
                ...
                dynamicParameters.Add("@error_source", entity.ErrorSource);
        ...
        public async Task<DashboardResponse> DashboardCount(DashboardCustomerRequestEntity entity, string storedProcedure)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var registrationDate = await _dbConnection.QueryFirstOrDefaultAsync<DateTime?>(
                        "SELECT TOP 1 RegistrationDate FROM tblCompanyinfo");

                    // Get the latest RenewDate from tblRenewCompany (if any)
                    var renewDate = await _dbConnection.QueryFirstOrDefaultAsync<DateTime?>(
                    ...
                    if ((hasRegistration || hasRenew) && !(registrationValid || renewValid))
                    {
                        ...
                        result.Status = (int)ResponseStatusCode.RefreshTokenExpired;
                        return result;
                    }
                }
                catch (Exception ex)
                {
                    logger.LogWarning(ex, "Failed to validate company subscription dates for dashboard.");
                }
        ...
        public async Task<ReleaseTempBookingViewEntity> ReleaseTempBooking(ReleaseTempBookingEntity entity, string storedProcedure)
        {
            ReleaseTempBookingViewEntity result = new ReleaseTempBookingViewEntity();
            ...
            try
            {
                ...
                // First Result Set (Booking Header)
                var booking = await data.ReadFirstOrDefaultAsync<ReleaseTempBookingViewEntity>();
